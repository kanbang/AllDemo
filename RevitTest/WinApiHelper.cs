...
namespace Warrentech.Velo.RevitUI
{
	class WinApiHelper
	{
		...
		public static IntPtr GetControlInptr(IntPtr mwh, string caption, IntPtr hwndChildAfter)
		{
			Console.WriteLine(string.Format("寻找{0}的句柄", caption));
			IntPtr tb = FindWindowEx(mwh, hwndChildAfter, null, caption);
			if (tb == IntPtr.Zero)
				throw new Exception(string.Format("找不到{0}", caption));
			else
				Console.WriteLine("Setting的句柄是:" + tb);
		...
		public static IntPtr GetControlInptr(IntPtr mwh, string caption,string className)
		{
			Console.WriteLine(string.Format("寻找{0}的句柄", caption));
			IntPtr tb = FindWindowEx(mwh, IntPtr.Zero, className, caption);
			if (tb == IntPtr.Zero)
				throw new Exception(string.Format("找不到{0}", caption));
			else
				Console.WriteLine("Setting的句柄是:" + tb);
